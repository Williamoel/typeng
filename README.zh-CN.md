...
和其他打字背单词的工具类似，我们提供常规的模式，有中文意思、词性、音标、自动语音（同时提供英音和美音）和英文释义。词库里的单词被词性划分为了更细致的词条，以保证用户可以充分学习不同词性的意思和用法。我们特别加入了对应词性的英文释义，因为考虑到高阶英语学习者不会只局限于记住中文含义，而是进一步根据英文释义来明确单词语境。对于用户拼写错误的单词，我们会马上反馈用户正确的拼写，并且等到用户完成所有新词的拼写后，再要求用户重新对之前错误拼写的单词进行拼写，直至所有本组单词被正确拼写一次。而拼写错误的单词可由用户自己选择进入错题本，供日后复习、巩固。

![模式选择](./images/mode_choice.png)

对于单词学习，我们尽可能为用户提供最大程度的个性化，用户可以随意选择接下来要学习的词条数目，也可以随意选择模式来匹配自己的偏好。其中cloze模式是我们区别于绝大多数单词学习工具的地方，绝大多数单词学习工具只会让用户自己去快速扫视例句，核心在记忆单词，但用户通常只停留在会认的阶段，而 typeng 会根据词条存在的例子挖空目标词汇或者它们的词形变体让用户拼写，用户因此可以充分了解词汇的语境和用法，甚至是词形变体。With cloze 模式是在常规单词拼写结束后再进行一轮语境拼写练习，而 Only in cloze 模式是只进行语境拼写练习，对于没有例子的单词则会退化到常规单词拼写。
...
为了让用户可以个性化制定自己的词库，我们提供高效的导入操作，用户可以通过导入 `Format Guide` 里面格式允许的 TXT 或者 CSV 文件来快速建立词库。考虑到我们提供的常规词库有很多基础单词，用户可以通过去重操作来删除过于简单或者其他词库已学习的词汇来加速学习，我们也同样提供大批量删除词条的快捷操作。

![自动填充](./images/auto_filling.png)

考虑到很多用户只想给出学习的单词，对应的例子甚至中文释义并不想填充，我们提供了自动填充的功能。对于选定范围的词汇，点击 Fill examples 之后，typeng 会根据词条的词性自动匹配合适的词典例子，对于少数单词或者词性没有例子匹配的情况，用户可以通过点击 Preview cleanup 来预览这些词条，自行选择是否保留这些词条内容。对于用户手动添加单词但没有输入对应中文释义的情况， typeng 会自动查找词典匹配词性的释义并补全，如果没有词性则会根据词性建立多个词条。
